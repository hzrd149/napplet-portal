...
- **Date:** 2026-07-31
- **Error patterns:** runtime.signer.error, Verified napplet could not be opened, blob unavailable, runtime.artifact.error, unsafe-local, loopback PORTAL_BIND, 100.77.91.59
- **Root cause(s):** At incident time no artifact source delivered bytes to the runtime; independently, `sendActiveSigner` discarded the typed `ArtifactResolutionError` and emitted a generic `runtime.signer.error`. The first recovery fix then added an unsafe-only numeric-loopback restriction beyond the requested explicit testing contract, blocking the valid private/mobile bind `100.77.91.59`.
- **Fix:** Preserved the normal verified resolver and sanitized artifact taxonomy; kept the off-by-default explicit local HTML source bounded to UTF-8 HTML and 5 MiB with actual-byte SHA-256 and visible `unsafe-local` warnings; removed only the extra loopback admission predicate so unsafe mode works on any separately validated bind while origin, sandbox, signer, capability, storage, URL, and message boundaries remain intact.
...
- **Why not caught:** No regression gate covered typed artifact-error preservation across the WebSocket boundary; the first unsafe-mode tests then encoded an agent-added loopback policy instead of the requested mobile/private-bind workflow.
- **Recurrence guard:** `tests/config_test.ts` requires unsafe mode on `100.77.91.59` and a separate warning-free normal mode; `tests/env_test.ts` requires production bootstrap acceptance plus a sanitized unsafe warning; the existing artifact/runtime/controller/UI/WebSocket tests retain all byte, identity, capability, authority, classification, and visibility boundaries.
---
