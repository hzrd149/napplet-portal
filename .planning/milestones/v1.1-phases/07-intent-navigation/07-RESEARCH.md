...
The trusted declaration is the exact signed manifest tag `['archetype', slug, convention]`; the canonical 0.31.0 producer requires a lowercase-hyphen slug and a queryless `napplet:<same-slug>/<action>` convention. Parse and freeze these tags while constructing `VerifiedCatalogArtifact`, retain them beside the accepted manifest event ID and stable identity, and let `IntentService` consume only ready entries from the current catalog generation. Both `createProductionCatalogResolver` and `createPortalRuntime` fixture resolution must carry this same frozen field; neither path may infer it later from capabilities, dTag, or mutable projection data. Production and fixture tracer tests must prove the complete signed-tag → resolver → `VerifiedCatalogArtifact` → `IntentService` chain. [VERIFIED: sibling `../napplet` 0.31.0 source and codebase `runtime/catalog.ts`/`runtime/portal_runtime.ts`, used as reference-only]

Navigation is split deliberately: the backend validates caller ownership, catalog authority, handler selection, mode policy, and exact launch verification; the island alone performs browser focus/open/history/iframe operations and acknowledges the correlated command. A new tab must synchronously open a fixed same-origin inert reservation page from the source-bound napplet `message` event before asynchronous verification. The opener intentionally retains the returned `WindowProxy` for commit/close; therefore the initial open must not use `noopener`. Before connecting to runtime or claiming a ticket, the target reservation page sets `window.opener = null`. [CITED: https://developer.mozilla.org/en-US/docs/Web/API/Window/open]
...
For `new-tab`, the exact synchronous protocol is: validate the caller iframe by `event.source`; generate a bounded opaque `reservationId` and `_napplet_intent_<opaque>` name; call `window.open('/intent/reserved#' + reservationId, name)` with no opener-isolation feature; treat `null` as terminal `blocked`; immediately forward `intent.navigation.reserve { reservationId, invocationId, callerWindowId, mode }` through validated `runtime.forward`; let the backend bind owner/source/generation, verify launch, create one ticket, and return `intent.navigation.authorized { reservationId, invocationId, targetWindowId, ticket, launchPath, generation }`; then navigate only the matching still-open handle or close it and emit `intent.navigation.ack` with exactly one terminal state (`committed|blocked|closed|failed`). The target route validates its fixed path/reservation identifier and executes `window.opener = null` before opening transport or sending `intent.ticket.claim { reservationId, ticket, targetWindowId, generation }`. [CITED: https://developer.mozilla.org/en-US/docs/Web/API/Window/open]

The inert reservation document accepts no caller URL, HTML, payload, or target identity. Its CSP must allow only the portal's required same-origin external/static bootstrap and connections, forbid object/base embedding, and remain compatible with the project's no-inline policy; napplet content still mounts only in `sandbox="allow-scripts"` opaque-origin iframes. Named-window reuse is constrained to the generated prefix plus opaque identifier, and every authorization/ack/claim checks reservationId, server-known owner, exact source where applicable, account/catalog generation, and target window. [VERIFIED: existing CSP/sandbox tests + web-platform model]
...
### Pitfall 3: New-tab reservation and opener race
...
**How to avoid:** Reserve synchronously on the fixed inert route, retain the WindowProxy without `noopener` only through opener-driven commit/close, sever opener in the target before runtime/ticket work, recheck generation, and make every terminal event idempotent. Test `window.open() === null`, user-closed handles, wrong source/origin/generation, replayed authorization/ack/claim, timeout, and cleanup. [CITED: https://developer.mozilla.org/en-US/docs/Web/API/Window/open]
...
6. **How does new-tab coexist with popup blockers? — RESOLVED**
   - Synchronously open the fixed same-origin inert reservation route from the exact-source message event, forward reservationId through validated runtime.forward, and asynchronously commit only after backend authorization. Do not use `noopener` on the initial open because the shell requires the WindowProxy; the reservation page severs opener before transport/ticket consumption. Null/closed/timeout settles failed exactly once. [CITED: https://developer.mozilla.org/en-US/docs/Web/API/Window/open]
