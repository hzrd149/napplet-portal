...
import type { RelayEventMessage, RelaySubscribeMessage } from "@napplet/nap/relay";
import { createPortalRuntime } from "../runtime/portal_runtime.ts";
import { loadRuntimeConfig } from "../runtime/config.ts";
import { startupSummary } from "../main.ts";
...
Deno.test("Fresh composition is singleton, loopback-safe, and starter-free", async () => {
  ...
  assert((main.match(/new App/g) ?? []).length === 1, "Fresh app must be singleton");
  assert(!main.includes("api2") && !main.includes("exampleLogger"), "starter routes must be gone");
  assert(!utils.includes("shared: string"), "starter request state must be gone");
  ...
  assert(summary.includes(fixture.coordinate), "summary may include coordinate");
  assert(!summary.includes("nsec") && !summary.includes("bunker://"), "summary must exclude secrets");
});
...
Deno.test("supplied Security Lab traverses verified mount, handshake, identity, and continuing stream", async () => {
  ...
  assert(artifact.dTag === fixture.identity.identifier, "verified dTag must bind session");
  assert(artifact.aggregateHash === fixture.identity.aggregateHash, "verified aggregate must bind session");
  ...
  window.receive(source, { type: "shell.ready" });
  assert(trace.filter((type) => type === "shell.init").length === 1, "source-bound init must be exact once");
  ...
  assert(messages[0]?.result.event.id === fixture.events.initial.id, "stored value must arrive");
  assert(messages[1]?.result.event.id === fixture.events.live.id, "live tail must continue");
  runtime.signOut();
  assert(runtime.activeAccount === null, "sign-out must remove signing authority");
});
