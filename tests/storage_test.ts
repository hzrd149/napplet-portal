...
Deno.test("missing snapshot initializes empty and canonical snapshot survives restart", async () => {
  ...
  try {
    ...
    await service.set({ ...identity, scope: "instance", instanceId: "b" }, "z", "last");
    ...
    assert(JSON.stringify(namespaceKeys) === JSON.stringify([...namespaceKeys].sort()), "namespaces serialize canonically");
    const restarted = await StorageService.open(new NappletStorageStore(temporary.path));
    assert(restarted.get(identity, "a") === "one", "shared value reloads");
    assert(restarted.get({ ...identity, scope: "instance", instanceId: "b" }, "z") === "last", "instance value reloads");
    assert(JSON.stringify(restarted.keys(identity)) === '["a","b"]', "restart key order is stable");
  } finally {
...
Deno.test("malformed snapshot fails closed and is not overwritten by mutation", async () => {
  ...
  for (const document of invalidDocuments) {
    ...
    try {
      ...
      assert(await Deno.readTextFile(temporary.path) === document, "malformed evidence remains unchanged");
    } finally {
      await Deno.remove(temporary.directory, { recursive: true });
    }
  }
...
Deno.test("concurrent cross-namespace mutations use one global snapshot tail", async () => {
  const temporary = await tempPath();
  try {
    const service = await StorageService.open(new NappletStorageStore(temporary.path));
    await Promise.all(Array.from({ length: 40 }, (_, index) => service.set(
      { ...identity, accountPubkey: String(index % 2).repeat(64) },
      `key-${index}`,
      `value-${index}`,
    )));
    const restarted = await StorageService.open(new NappletStorageStore(temporary.path));
    assert(restarted.keys({ ...identity, accountPubkey: "0".repeat(64) }).length === 20, "first namespace retained");
...
Deno.test("write failure preserves committed memory and disk and queue recovers", async () => {
  ...
  try {
    ...
    assert(service.get(identity, "unstable") === null, "memory does not publish failed write");
    assert(JSON.stringify(await store.read()) === JSON.stringify(committed), "disk remains committed");
    fail = false;
    await service.set(identity, "recovered", "after");
    assert(service.get(identity, "recovered") === "after", "queue accepts later work");
  } finally {
    await Deno.remove(temporary.directory, { recursive: true });
  }
});
