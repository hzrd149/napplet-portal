...
Deno.test("detach revokes immediately while origin expires after grace", () => {
  const effects: Array<{ recipient: MediaActorRef; message: { type: string } }> = [];
  ...
  const registry = new ConnectionRegistry({
    ...
    detachConnection: (connectionId) => coordinator.detach({ ...actor, connectionId }),
  ...
  assert(coordinator.current("account")?.owner === null, "detach revokes owner now");
  assert(!coordinator.current("account")?.terminal, "origin remains during grace");
  const resumed = registry.attach(() => {}, attached.reconnectToken);
  assert(resumed.resumed, "connection resumes inside grace");
  assert(coordinator.current("account")?.owner === null, "resume never restores ownership");
  registry.detach(attached.connectionId);
  clock.flush();
  assert(coordinator.current("account") === null, "origin expiry removes active session");
});
...
Deno.test("account change and shutdown stop before terminal snapshot", () => {
  ...
  assert(messages[0]?.type === "media.command", "account change sends stop first");
  assert(messages[1]?.type === "runtime.media.snapshot", "terminal snapshot follows stop");
  ...
  assert(messages[1]?.type === "runtime.media.snapshot", "shutdown terminalizes after stop");
});
...
Deno.test("owner send failure commits newer ownerless generation", () => {
  ...
  const coordinator = new MediaSessionCoordinator({
    deliver: (_recipient, message) => !(failCommand && message.type === "media.command"),
  });
  coordinator.connect("account", actor);
  ...
  assert(current!.generation > created.session!.generation, "failure advances generation");
});
