...
Deno.test("authoritative media shell tracer gates snapshots and stale ownership", () => {
  ...
  assert(
    shell.forward({ type: "media.state", sessionId: "session-1", status: "playing" }),
  ...
  assert(stops === 1 && !shell.isOwner, "authority loss stops local playback first");
});
...
Deno.test("authoritative media shell tracer exposes truthful autoplay retry", async () => {
  ...
  assert(await shell.play(() => Promise.resolve()), "fulfilled play is truthful");
  assert(reports.at(-1)?.message &&
    (reports.at(-1)!.message as Record<string, unknown>).status === "playing",
  "playing reports only after fulfillment");
  assert(!await shell.play(() => Promise.reject(new DOMException("blocked", "NotAllowedError"))),
...
Deno.test("authoritative media shell tracer renders bounded accessible controls", () => {
  ...
  assert(html.includes("Tap to play"), "autoplay retry is explicit");
